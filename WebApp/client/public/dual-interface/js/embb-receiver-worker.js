 ...
 *   2. If found: extract seqNo + t3, strip stamp from frame data, record t4Wall.
 *   3. Pass the (clean) frame to the decoder.
 *   4. postMessage the timing data to the main thread.
 *      t4Perf (performance.now() timebase) is recorded by the main thread's
 *      onmessage handler to avoid Worker/main timebase mismatch.
 *
 * Stamp layout (14 bytes appended by EmbbEncodeTimestamper.cs):
 *   [0-1]  0x4C 0x54  magic 'LT'
 *   [2-5]  uint32 seqNo  (LE)
 *   [6-13] float64 t3 (server high-precision wall clock ms)
 */
    ...
    transform(encodedFrame, controller) {
      ...
      if (data.length >= STAMP_SIZE) {
        ...
        if (data[stampStart] === MAGIC_0 && data[stampStart + 1] === MAGIC_1) {
          const t4Wall = Date.now(); // wall clock for eMBB TX (NTP cross-VM)

          const view  = new DataView(data.buffer, data.byteOffset + stampStart);
          const seqNo = view.getUint32(2, true);
          const t3    = view.getFloat64(6, true);

          encodedFrame.data = data.slice(0, stampStart).buffer;
          // t4Perf is NOT recorded here – Worker performance.now() has a different
          // timeOrigin than the main thread. The main thread records it in onmessage.
          self.postMessage({ type: 'latency-stamp', seqNo, t3, t4Wall });
        }
      }
